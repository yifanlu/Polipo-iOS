...
static void
default_arguments (int port, const char *hostname, int host_port, Arguments *arg)
{
  /* defaults */
  
  arg->me = "htc";
  arg->device = NULL;
  arg->forward_port = port;
  arg->host_name = hostname ? strdup(hostname) : NULL;
...
parse_arguments (int argc, char **argv, Arguments *arg)
{
  ...
  default_arguments(-1, NULL, DEFAULT_HOST_PORT, arg);
...
int
httptunnel_setdebug (int loglevel, FILE *logfile)
{
#ifdef DEBUG_MODE
...
int
httptunnel_client (int port, const char *hostname, int host_port)
{
  int s = -1;
  int fd = -1;
  Arguments arg;
  ...
  default_arguments (port, hostname, host_port, &arg);
